...
class MOlap_rawatinap_penyakit extends CI_Model {
 // db2 digunakan untuk mengakses database ke-2
	...
	public function penyakit($tgl_masuk)
	{
		...
		$jk = array('P', 'L');
	 	$apr_labor_asuransi = $this->db2->select('nama_penyakit as id_penyakit, COUNT(fact_rawat_inap.id_penyakit) AS jml_penyakit')
	 					   ->join('dim_penyakit','fact_rawat_inap.id_penyakit=dim_penyakit.id_penyakit')
						   ...
						   ->where('tgl_masuk <=', $akhir)
	 					   ...
	 					   ->group_by('fact_rawat_inap.id_penyakit')
